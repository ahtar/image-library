...
describe('Коллекции', () => {
    ...
    it('Новая коллекция создается', () => {
        cy.readFile('./tests/e2e/dummy.jpg', null, {log: true}).then((obj: Blob) => {
        ...
        cy.get('[data-test="collection-create-name"] input').type('cypressCollection');
        cy.get('[data-test="collection-create-theme"] input').type('cypress collection theme');
        cy.get('[data-test="collection-create-description"] textarea').type('cypress collection description');

...
describe('FormImageView.vue', () => {
    ...
    it('изображение рендерится', async () => {
        ...
        expect(wrapper.find<HTMLImageElement>('[data-test="form-view-viewer"] img').element.src).not.toBe('');
    });
    ...
    it('активное изображение меняется', async () => {
        ...
        const firstSrc = wrapper.find<HTMLImageElement>('[data-test="form-view-viewer"] img').element.src;
        
        expect(firstSrc).not.toBe('');

        //нажимаем на 2 изображение в сете
        await userEvent.click(wrapper.find('[data-test="form-view-select"]').element.children[1]);

        //img.src должен измениться на новое изображение
        expect(wrapper.find<HTMLImageElement>('[data-test="form-view-viewer"] img').element.src).not.toBe('');
        expect(wrapper.find<HTMLImageElement>('[data-test="form-view-viewer"] img').element.src).not.toBe(firstSrc);
    });
});
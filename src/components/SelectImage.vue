<template>
    <div class="select-images" @drop="drop($event)" @dragenter.prevent @dragover.prevent v-if="draggable">
        <card-image-small v-for="(card, i) in set" draggable="true" data-test="select-image-card"
            @dragstart="drag($event, card)" @dragover="dragOver($event, card)" :key="card.manifest.id" :image="card"
            @click="change(card, i, $event)" :id="i" />
    </div>
    <div class="select-images" v-else>
        <card-image-small v-for="(card, i) in set" data-test="select-image-card" :key="card.manifest.id" :image="card"
            @click="change(card, i, $event)" :id="i" />
    </div>
</template>
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        function drop(event: DragEvent) {
            ...
            const item = (props.set as ImageSingle[]).find(
                (image) => image.manifest.id == id
            );

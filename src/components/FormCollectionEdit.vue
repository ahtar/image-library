<template>
    <modal-dark @close="store.close" data-test="modal">
        <div class="content-wrapper" data-test="collection-edit-wrapper">
            <input-text class="wrapper-section" label="Name" placeholder="Collection name" :important="true" v-model="data.name" data-test="collection-edit-name"/>
            <input-text class="wrapper-section" label="Theme" placeholder="Collection theme" v-model="data.theme" data-test="collection-edit-theme"/>
            <input-text class="wrapper-section input-description" label="Description" placeholder="Collection description" v-model="data.description" :textarea="true" data-test="collection-edit-description"/>
            <div class="wrapper-section section-checkbox">
                <input-checkbox v-model="store.form.options.corrupted" label="Corrupted" data-test="collection-edit-corrupted"/>
            </div>
            <div class="wrapper-section button-container">
                <button-small @click="store.save" :blocked="!buttonSaveActive" data-test="collection-edit-save">Save</button-small>
...
<script lang="ts">
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            async function fix() {
                if(!store.collection?.loaded) await store.collection?.initLoadCollection();
                ...
                for(const imageSet of sets!) {
                    for(const image of (imageSet as ImageSet).arr) {
                        const double = store.collection!.arr.find((i) => i.manifest.id == image.manifest.id);
                        if(double) {

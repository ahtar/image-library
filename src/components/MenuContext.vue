    ...
    export default defineComponent({
        ...
        setup(props, {emit}) {
            ...
            onMounted(() => {
                ...
                if(props.event.clientY + menu.value!.offsetHeight > document.body.clientHeight) {
                    menu.value!.style.top = `${props.event.clientY - menu.value!.offsetHeight}px`;
                } else {
                    menu.value!.style.top = `${props.event.clientY}px`;
                }

                //рассчитать X координату с учётом границ экрана
                if(props.event.clientX + menu.value!.offsetWidth > document.body.clientWidth) {
                    menu.value!.style.left = `${props.event.clientX - menu.value!.offsetWidth}px`;
                } else {

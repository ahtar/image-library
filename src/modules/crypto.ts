 ...
 */
 async function corrupt(data: Blob | ArrayBuffer) {
    ...
    const mergedArray = concatTypedArrays([falseDataOne, firstPart, falseDataTwo, secondPart]);
...
async function isCorrupted(data: Blob | ArrayBuffer) {
    ...
    const firstPart = uint8Array.slice(0, 4).toString() == [255, 255, 50, 50].toString();
    const secondPart = uint8Array.slice(n + 4, n + 4 + 7).toString() == [255, 110, 1, 1, 5, 10, 255].toString();

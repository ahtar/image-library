...
export default function () {
            ...
            (n, o) => {
                if (n.length > o.length) {
                    ...
                    for (const image of removedImages) {
                        const index = displayedImages.value.findIndex(
    ...
    function displayNextImages(count = 15) {
        if (displayedImages.value.length >= displayableImages.value.length)
            return false;
        const temp = displayableImages.value.slice(
            displayedImages.value.length,
            displayedImages.value.length + count
        );
